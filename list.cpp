...
{ 					//List object as a parameter and creates a deep copy of its elements.
	...
	while (count < list.size()) {    // Use a loop to iterate through each element in the source list "list."
		push_back(list.getAt(count)); // Call the "push_back" method to add a copy of the element at index "count"
...
{
	head = list.head;    // Transfer ownership of the "head" pointer from the source list to the current list.
	tail = list.tail;		// Transfer ownership of the "tail" pointer from the source list to the current list.
	asc_head = list.ask_head;    // Transfer ownership of the "asc_head" pointer from the source list to the current list.
	desc_head = list.desk_head;    // Transfer ownership of the "desc_head" pointer from the source list to the current lis.
	...
	list.desc_head = nullptr;    // Set the "asc_head" pointer in the source list to nullptr to indicate ownership transfer.
	list.asc_head = nullptr;    // Set the "desc_head" pointer in the source list to nullptr to indicate ownership transfer.
}

template <typename T>	// Initaializer listt constructor
...
List<T>::List(size_t init_size, const T& data) : 
	head{nullptr}, 
	tail{nullptr}, 
	asc_head{nullptr}, 
	desc_head{nullptr}, 
...
void List<T>::unique()
{
	...
	while (current && current->greater) {
		if (current->data == current->greater->data) {
			removeNode(current);
		}		
...
template <typename T>	// Traverse the List in sorted order 
void List<T>::traverseSorted(void (*visit)(const T& d), bool assending) const
//and apply the provided 'visit' function to each element.
{
	Node* current = assending ? asc_head : desc_head;
// Initialize a pointer 'current' based on the sorting order (ascending or descending).
	while (current) {
// Traverse the List while following the specified sorting order and call the 'visit' function for each element.
		visit(current->data);
		current = assending ? current->greater : current->lesser;
// Update 'current' based on the sorting order (ascending or descending).

	}
}

template <typename T>
void List<T>::reverse()		//Reverses the container swaping the head and the tail
{
	std::swap(tail, head);
}
...
template <typename T>
void List<T>::removeNode(Node* node)     // Remove a specified node in the list.
{
	if (!node) {    // Check if a node has valid value.
		throw std::invalid_argument("The mentioned argument is invalid");
	}
	if (node == head) {    // If node is the head it's the first element; use 'pop_front' to remove it.
		pop_front();
	} else if (node == tail) {    // If node is the last element, use 'pop_back' to remove it.
		pop_back();
	} else {    // If node  somewhere in the middle of the list,
		if (node == asc_head) {        // Update the ascending and descending pointers if necessary.
			asc_head = node->greater;
		}
		if (node == desc_head) {
			desc_head = node->lesser;
		}
		if (node->lesser) {        // Update the pointers of neighboring nodes.
			node->lesser->greater = node->greater;
		}
		if (node->greater) {
			node->greater->lesser = node->lesser;
		}
		node->prev->next = node->next;        // Adjust the pointers to remove node from the linked list.
		node->next->prev = node->prev;
		delete node;        // Deallocate memory for node and set it to nullptr.
		node = nullptr;
		--size_;
	}        // Decrease the size of the list to reflect the removal of an element.
}
